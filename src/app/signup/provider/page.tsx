...
export default function ProviderSignup() {
  ...
  const handleSubmit = async (data: {
    ...
    try {
      ...
      router.replace('/onboarding/provider/1');
    } catch (err) {
      ...
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          ...
          <h1 className="mt-6 text-3xl font-bold tracking-tight text-black">
            {t('auth.signup.provider.title')}
          </h1>
          <p className="mt-2 text-black">
            {t('auth.signup.provider.subtitle')}
          </p>
        </div>
        ...
        <SignupForm defaultRole="provider" onSubmit={handleSubmit} />
        ...
        <p className="text-center text-sm text-black">
          ...
          <Link href="/login?role=provider" className="text-violet-600 hover:text-violet-700">
            {t('auth.common.login')}
          </Link>
        </p>
      </div>
